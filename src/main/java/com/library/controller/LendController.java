...
public class LendController {
    ...
    public String deleteBook(HttpServletRequest request, RedirectAttributes redirectAttributes) {
        ...
        if (bookService.deleteBook(bookId)) {
            redirectAttributes.addFlashAttribute("succ", "書籍削除完了！");
        } else {
            redirectAttributes.addFlashAttribute("error", "書籍削除失敗！");
        }
    ...
    public String deleteLend(HttpServletRequest request, RedirectAttributes redirectAttributes) {
        ...
        if (lendService.deleteLend(serNum) > 0) {
            redirectAttributes.addFlashAttribute("succ", "記録削除完了！");
        } else {
            redirectAttributes.addFlashAttribute("error", "記録削除失敗！");
        }
    ...
    public String bookLend(HttpServletRequest request, RedirectAttributes redirectAttributes) {
        ...
        if (lendService.lendBook(bookId, readerId)) {
            redirectAttributes.addFlashAttribute("succ", "書籍貸出完了！");
        } else {
            redirectAttributes.addFlashAttribute("succ", "書籍貸出失敗！");
        }
    ...
    public String bookReturn(HttpServletRequest request, RedirectAttributes redirectAttributes) {
        ...
        if (lendService.returnBook(bookId, readerId)) {
            redirectAttributes.addFlashAttribute("succ", "書籍返却完了！");
        } else {
            redirectAttributes.addFlashAttribute("error", "書籍返却失敗！");
        }
        return "redirect:/reader_books.html";
    }
}
